...
<form method="POST" action="{{ route('wizard.paso4') }}">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="mb-4">
            ...
            <input type="{{ in_array($name, ['presion_arterial', 'evaluacion_pie_dm']) ? 'text' : 'number' }}"
                name="{{ $name }}"
                step="any"
                value="{{ old($name, $evaluacion[$name] ?? '') }}"
                class="w-full border rounded px-3 py-2 border-gray-300">
    ...
    <div class="mb-4">
        ...
        <select name="test_morisky_green"
            class="w-full border rounded px-3 py-2 border-gray-300">
    ...
    <div class="mb-4">
        ...
        <input type="date" name="control_renal_fecha"
            value="{{ old('control_renal_fecha', $evaluacion['control_renal_fecha'] ?? '') }}"
            class="w-full border rounded px-3 py-2 border-gray-300">
        @error('control_renal_fecha')
            <p class="text-red-500 text-sm">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="date" name="actividad_fecha"
            value="{{ old('actividad_fecha', $actividad['fecha'] ?? '') }}"
            class="w-full border rounded px-3 py-2 border-gray-300">
        @error('actividad_fecha')
            <p class="text-red-500 text-sm">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="text" name="numero_sesion"
            value="{{ old('numero_sesion', $actividad['numero_sesion'] ?? '') }}"
            class="w-full border rounded px-3 py-2 border-gray-300">
        @error('numero_sesion')
            <p class="text-red-500 text-sm">{{ $message }}</p>
        @enderror
