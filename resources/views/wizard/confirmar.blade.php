<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto p-6 bg-white rounded shadow">
            ...
            <div class="mb-8 border border-gray-200 rounded p-4 bg-gray-50">
                ...
                <div class="mb-8 border rounded p-4 {{ $fondoEtiqueta }}">
                    ...
                    @if (is_array($datos))
                            ...
                            @foreach ($datos as $index => $item)
                                ...
                                <div class="mb-4 p-3 border rounded {{ $estilos['fondo'] }}">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        @if ($estado)
                                            <span
                                                class="text-xs px-2 py-1 {{ $estilos['clase'] }} rounded font-medium">{{ $estilos['texto'] }}</span>
                                        @endif
                                    </div>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                                        @php
                        ...
                        @endif
                    @else
                        <p class="text-gray-500 italic bold text-lg ml-5">Sin datos registrados.</p>
                    @endif
                </div>
            </div>
        @endforeach
        ...
        <div class="mb-8 border border-gray-200 rounded p-4 bg-gray-50">
            ...
            {{-- Botones --}}
            <form
                action="{{ isset($adulto_id) && $adulto_id ? route('wizard.finalizar', ['adulto_id' => $adulto_id]) : route('wizard.finalizar') }}"
                method="POST" class="mt-10 flex justify-between">
                @csrf
                ...
                @if (session()->has('adulto_id'))
                    <button type="submit"
                        class="bg-yellow-600 text-white px-6 py-2 rounded hover:bg-yellow-700 flex items-center">
                ...
                @else
                    <button type="submit"
                        class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-800 flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
