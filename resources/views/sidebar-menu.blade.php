{{-- Sidebar fijo --}}
<aside
    class="fixed left-0 top-16 w-64 h-[calc(100vh-4rem)] bg-white border-r border-gray-200 transform -translate-x-full lg:translate-x-0 transition-transform duration-200 ease-in-out z-40 lg:z-40">
    <!-- Header del Sidebar -->
    ...
    <nav class="px-4 pt-4 overflow-y-auto h-full pb-20">
        <div class="space-y-1">
            <a href="{{ route('dashboard') }}"
                ...
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('dashboard') ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500' }}"
            ...
            <a href="{{ route('adultos.index') }}"
                class="group flex items-center px-3 py-2.5 text-sm font-medium {{ request()->routeIs('adultos.index') ? 'text-sky-600 bg-sky-100 0 border-l-3 border-sky-500' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50 border-l-3 border-transparent hover:border-blue-500' }} transition-all duration-200">
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('adultos.index') ? 'text-sky-500' : 'text-gray-400 group-hover:text-sky-500' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('adultos.create') }}"
                ...
                <svg class="w-5 h-5 mr-3 {{ request()->routeIs('adultos.create') ? 'text-green-500' : 'text-gray-400 group-hover:text-green-500' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if(auth()->user()->is_admin)
                <a href="{{ route('users.index') }}"
                    class="group flex items-center px-3 py-2.5 text-sm font-medium {{ request()->routeIs('users.*') ? 'text-purple-600 bg-purple-200 border-l-3 border-purple-500' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50 border-l-3 border-transparent hover:border-purple-500' }} transition-all duration-200">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('users.*') ? 'text-purple-500' : 'text-gray-400 group-hover:text-purple-500' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
